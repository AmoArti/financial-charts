...
import {
  ...
  describe('trimMultiData', () => {
    ...
    it('should handle empty datasets array', () => {
      ...
      const expected: MultiDatasetStockData = { labels: [], datasets: [] };
      const result = trimMultiData(data);
      expect(result).toEqual(expected);
    });
  
     it('should handle datasets with empty values array', () => {
       const data: MultiDatasetStockData = {
         labels: ['2021', '2022'],
         datasets: [{ label: 'Revenue', values: [] }]
       };
       const expected: MultiDatasetStockData = { labels: [], datasets: [{ label: 'Revenue', values: [] }] };
       const result = trimMultiData(data);
       expect(result).toEqual(expected);
     });
     ...
     it('should return original data if first dataset values are not zero but labels are empty', () => {
       const data: MultiDatasetStockData = {
         labels: [],
